...
$d= $whereattached .  " " . $orderby;
		  ...
		  if($search==""){
					 $sql="select * from jtechel.mywork  " . $c; 					
	                 $sqlcon = "select * from jtechel.mywork  "  . $d; 
		  }
			else						 
	   ...
	   // 필드별 검색하기
			...
			$sql = "select * from jtechel.mywork where ($where_clause) $a";
			$sqlcon = "select * from jtechel.mywork where ($where_clause) $b";
		  } 
...
<?php  include $_SERVER['DOCUMENT_ROOT'] . '/mywork/navbarsub.php'; ?>
...
<div class="container-fluid">	
     ...
     <div class="card-body justify-content-center text-center">				          
			<span class="fs-4 text-center">		수주/발주 리스트 </span>	
     <div class="d-flex col-sm-12 p-2 m-2 mt-3 justify-content-center">					         				 
			...
			<span class="fs-6 mb-1">  
			   ...
			   <span id="showspecialdate" class="btn btn-dark btn-sm " > 기간 </span>	&nbsp; 
				<div id="showspecialframe" class="card text-center  justify-content-center" >
				...
				<span class="text-center">   ▷ 총 <?= $total_row ?> Data </span>		
				</div>			 
			 ...
			 <div class="row col-sm-12">				
					...
					<input type="text" id="search" name="search" value="<?=$search?>" onkeydown="SearchEnter(event);"  > &nbsp;
                    <button id="searchBtn" type="button" class="btn btn-dark  btn-sm"  ><ion-icon name="search-outline"></ion-icon></button> 
					</span>
...
<?php if ($chkMobile): ?>
    <!-- 모바일 환경일 때 보이는 버튼 -->
	<div class="row col-sm-12 ">	
       <button type="button" class="btn-primary btn-block" onclick="toggleColumns();">기타 컬럼 보이기/숨기기</button>
	</div>
    <!-- 나머지 컬럼에 대한 버튼 추가 -->
<?php endif; ?>	
 ...
 <div class="row col-sm-12 table-responsive">	
	<table class="table table-bordered table-striped table-hover " style="cursor:pointer;"> 	
          <thead class="table-secondary">
				...
				<th class="col text-center col-group1">No</th>
				<th class="text-center  text-center col-group2" id="registerBtn" >접수</th>
				<th class="text-center col-group2" id="measuredayBtn" >실측</th>
				<th class="text-center text-primary col-group2"  id="forecastBtn" >예정</th>
				<th class="text-center text-success col-group2"  id="workdoneBtn" >완료</th>
				<th class="text-center col-group2"  id="demandBtn" >청구</th>
				<th class="text-center col-group2"  id="donedemandBtn" >지급</th>
				<th class=" text-center col-group2"> PJ num </th>
				<th class=" text-center col-group1"> 공사명 </th>
				<th class=" text-center col-group2">주소</th>            
				<th class=" text-center col-group2">원청</th>
				<th class=" text-center col-group1">발주처</th>
				<th class="text-center col-group2">시공팀</th>
				<?php if($level < 2)
				   print '<th class="text-secondary text-center col-group2">세부발주</th>';
				 ?>
				 <th class=" text-secondary text-center col-group2">시공팀 기록</th>
          </tr>	  			  
		  </thead>
	  <tbody>
	<?php  
		  if ($page<=1)  
	       ...
	       <tr onclick="redirect('write_form.php?num=<?=$num?>&page=<?=$page?>&scale=<?=$scale?>')">
            ...
            <td class="text-center col-group1"><?=$start_num?> </td>
            <td class="text-center col-group2"> <?=$regist_day?>  </td>
            <td class="text-center col-group2"> <?=$measureday?> </td>
            <td class="text-center text-primary col-group2"> <?=$workday?> </td>
            <td class="text-center  text-success col-group2">  <?=$doneday?> </td>
            <td class="text-center  col-group2">  <?=$demand?> </td>
            <td class="text-center  col-group2">  <?=$donedemand?> </td>
            <td class="  col-group2"> <?=$pjnum?> </td>
            <td class="  col-group1"> <strong> <?=$workplacename?>  </strong> </td>
            <td class="  col-group2">		<?=$address?> 		</td> 
            <td class=" text-center  col-group2">	<?=$firstord?> 	</td>	
            <td class=" text-center  col-group1">	<?=$secondord?> 	</td>	
            <td class="text-center  col-group2">		<?=$worker?> 		</td>
			<?php if($level < 2)
             print '<td class=" text-secondary col-group2 ">' .  iconv_substr($memo,0,15,"utf-8") . '	</td>	 ';
		    ?>
			<td class=" text-secondary col-group2">		 <?=iconv_substr($memo2,0,15,"utf-8")?> 	</td>
...
<script>
let isGroup1Only = false;  // 초기에는 col-group1만 보이게 설정

// 페이지 로드 시 실행되는 함수
window.onload = function() {
    if (window.innerWidth <= 768) {  // 모바일 화면 크기 확인
        toggleColumns();  // 모바일 화면에서만 초기 상태 설정
    }
};
...
function toggleColumns() {
    ...
    let group1Elements = document.getElementsByClassName('col-group1');
    let group2Elements = document.getElementsByClassName('col-group2');


    if (isGroup1Only) {
        // 모든 컬럼을 보이게 설정
        for (let i = 0; i < group1Elements.length; i++) {
            group1Elements[i].style.display = '';
        }
        for (let j = 0; j < group2Elements.length; j++) {
            group2Elements[j].style.display = '';
        }
    } else {
        // 'col-group1' 컬럼만 보이게 설정
        for (let i = 0; i < group1Elements.length; i++) {
            group1Elements[i].style.display = '';
        }
        // 'col-group2' 컬럼을 숨김
    ...
    isGroup1Only = !isGroup1Only;  // 상태 토글
}

// 화면 크기가 변경될 때마다 실행되는 함수
window.onresize = function() {
    if (window.innerWidth > 768 && isGroup1Only) {
        // PC 화면으로 전환 시 모든 컬럼 보이게 설정
        toggleColumns();
    }
};
