...
require_once("../lib/mydb.php");
 ...
 if(isset($_REQUEST["page"])) // $_REQUEST["page"]값이 없을 때에는 1로 지정 
  ...
  $first_num = ($page-1) * $scale;  // 리스트에 표시되는 게시글의 첫 순번.	 
	...
	if(!empty($search)) {
		$sql = "select * from jtechel." . $DB . " where part = 'mywork' and (id like '%$search%' or name like '%$search%' or nick like '%$search%') order by id desc limit $first_num, $scale";
		$sqlcon = "select * from jtechel." . $DB . " where part = 'mywork' and (id like '%$search%' or name like '%$search%' or nick like '%$search%') order by id desc";
	} else {
		$sql = "select * from jtechel." . $DB . " where part = 'mywork' order by id desc limit $first_num, $scale";
		$sqlcon = "select * from jtechel." . $DB . " where part = 'mywork' order by id desc";
	}
