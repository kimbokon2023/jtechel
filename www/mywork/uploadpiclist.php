...
<div class="card p-2">    
<div class="card-header p-2 mb-2  justify-content-center ">    
	<?php if ($chkMobile): ?>
    <!-- 모바일 환경일 때 보이는 내용 -->
	<div class="row mt-2 mb-2 justify-content-center">    
	 <span>
	 <span class="badge bg-primary fs-6 text-center me-2"> &nbsp; 사진대지 리스트 목록만들기 </span>
	 <button  type="button" class="btn btn-secondary btn-sm" id="saveBtn"> 일괄등록 </button>	 
	 </span>

<?php else: ?>		
    <!-- PC 환경일 때 보이는 내용 -->						
	<div class="d-flex mt-2 mb-2 justify-content-center">    
	 <span class="badge bg-primary <?=$fs?> text-center me-2"> &nbsp; 사진대지 리스트 목록만들기 </span>
	...
	<table class="table table-bordered" id="dynamicTable">
		<thead>
			<tr>
				...
				<th style="width:60%;">사진대지 목록 리스트</th>
...
$(document).ready(function() {
    ...
    $(document).on('click', '.add', function() {
        ...
        var newRow = '<tr>' +
            ...
            '<td><button type="button" class="btn btn-success btn-sm add">+</button> ' +
    ...
    if (!piclistObj.col2 || piclistObj.col2.length === 0) {
        $('#dynamicTable tbody').append('<tr id="row1">' +
            ...
            '<td><input type="text" name="col2[]" class="form-control" /></td>' +
            '<td><button type="button" class="btn btn-success  btn-sm add">+</button></td>' +
        '</tr>');
    }	
