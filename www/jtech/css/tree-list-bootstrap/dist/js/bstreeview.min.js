...
var l={expandIcon:"fa fa-angle-down",
...
a(".bstreeview").on("click",".list-group-item",function(){
	// 펼쳐보이는 아이콘 조정
	 ...
	 a(".state-icon",this).toggleClass(b.settings.expandIcon).toggleClass(b.settings.expandIcon)
	 }	 
	)},
	initData:function(b){
		if(b.nodes){var f=this;a.each(b.nodes,function(a,c){
	...
	build:function(b,f,h){
		...
		a.each(f,function(f,d){
			...
			var e=a('<div href="#itemid" id="tree_' + d.nodeID + '" data-depid="' + d.seq + '" class="list-group-item"></div>').attr("href",
				"#"+c.itemIdPrefix + d.nodeId).attr("style","padding-left:" + g);
	...
	if(d.nodes){
		...
		e.append(d.text);
		// attr onClick추가(수정)
		// attr onClick추가(수정)  - 클릭했을때 이벤트 넣기
		e.append('<a href="#" onclick="node_select(' + '\'' + d.text + '\'' + ')" style="float:right;" > <span class="badge bg-info"> 수정 <span> </a>');
