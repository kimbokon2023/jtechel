	...
	for ($i = 1; $i <= 150; $i++) {
		...
		if ($isEditMode) {
			...
			$sum = intval($output_value1) + intval($output_value2) + intval($output_value3) + intval($output_value4) + intval($output_value5);
                ...
                echo '<td  class="text-center"><input type="text" name="input_amount[]" inputmode="numeric" pattern="[0-9]{1,4}"  class="form-control input-amount text-center" value="' . $input_value . '" ></td>';                
                ...
                for ($j = 1; $j <= 5; $j++) {
                    echo '<td class="text-center"><input type="text" name="output_amount' . $j . '[]" inputmode="numeric" pattern="[0-9]{1,4}"  class="form-control output-amount' . ($j + 1) . '  text-center"></td>';
                }
            }
...
// 빈칸 넣음 
	$("#nullButton").click(function(){ 			
	  ...
	  if (inputFieldInfo.inputField) {
			...
			// 모달 창 닫기
			var modal = bootstrap.Modal.getInstance(document.getElementById('nameModal'));
			if (modal) {
				setTimeout(function () {
				  modal.hide();
				}, 800);
...
var markCheckboxes = document.querySelectorAll('.mark-check');
  ...
  document.querySelectorAll('input[type="text"]').forEach(function (input) {
    input.addEventListener('input', function () {
      if (this.classList.contains('namearr_arr1') || this.classList.contains('namearr_arr2') || this.classList.contains('namearr_arr3') || this.classList.contains('namearr_arr4') || this.classList.contains('namearr_arr5') || this.classList.contains('name-input')) {
        // namearr_arr 입력 요소의 숫자, 영문자, 한글, 하이픈(-) 입력 허용
      ...
      if (this.value === 'undefined') {
        this.value = ''; // undefined 값을 공백으로 변경
      }
...
function OnlyCalculate()
{
  ...
  var outputValues = Array.from({ length: 5 }, (_, i) => getValuesFromElements('output_amount' + (i + 1) + '[]'));
  ...
  var mcount = Math.max(nonEmptyInputValues.length, nonEmptyOutputValues.length);
	...
	for (let i = 1; i <= 6; i++) {  // 지출항목 6개 줄.. 간식추가함
	  var input = document.querySelector('input[name="text_arr' + i + '[]"]:first-of-type') || '';
	  var value = input.value || '';
...
// 전화번호 이름 검색을 위한 검색조건 설정
function initializeProgram() {

		// 각 입력 필드에 클릭 이벤트 리스너 추가
		...
		fetch(URL, {
		  ...
		  .then(guest_data => {
			// console.log("console.log(JSON.stringify(" + JSON.stringify(guest_data));
	...
	// names 배열에 이름과 전화번호 조합하여 추가
	names = guestNames.slice(0, 10).map(function(name, index) {
	  var tel = guestTels[index] || ''; // 전화번호가 없을 경우 공백 처리
	  return name + ' ' + tel;
	});
	...
	nameSearchInput.addEventListener('input', function () {
	  ...
	  if (searchText === '') {
		...
		for (var i = 0; i < guestNames.length; i++) {
		  var name = guestNames[i];
		  var tel = guestTels[i] || ''; // 전화번호가 없을 경우 공백 처리
		  if (name.toLowerCase().includes(searchText) || tel.includes(searchText)) {
			names.push(name + ' ' + tel);
		  }
		}
	  ...
	  names.forEach(function (name) {
		...
		listItem.className = 'name-item'; // 리스트 아이템 클래스 추가
		...
		listItem.addEventListener('click', function () {
		  var clickedName = nameElement.textContent;
		  if (inputFieldInfo.inputField) {
			// 클릭한 이름을 해당 위치의 입력 필드에 설정
			inputFieldInfo.inputField.value = clickedName;
		...
		nameResult.appendChild(listItem);
	  });
	});
