<?php session_start();
isset($_REQUEST["companycount"]) ? $companycount=$_REQUEST["companycount"] : $companycount=0;   
 ?>
...
<style>
   @import url("https://cdn.jsdelivr.net/npm/bootstrap-icons@1.7.1/font/bootstrap-icons.css");
	...
	<section class ="d-flex fex-column align-items-left flex-md-row p-1">
	 ...
	 try{
		  $sql = "select * from mirae8440.steelcompany ";
...
$(document).ready(function(){
	...
	let timer2 = setTimeout(function(){  // 시작과 동시에 계산이 이뤄진다.
				...
				// grid 변경된 내용을 php 넘기기 위해 input hidden에 넣는다.
					function savegrid() {		
					// 납품회사 숫자
					var companycount = <?php echo $companycount; ?> ; 					
						// 부모창 select 옵션제거 제이쿼리 기법
						$("#company option", opener.document).remove();
					        ...
					        // console.log(grid.getRowCount());	//삭제시 숫자가 정상적으로 줄어든다.
							 ...
							 for(i=0;i<MAXcount;i++) {      // grid.value는 중간중간 데이터가 빠진다. rowkey가 삭제/ 추가된 것을 반영못함.    
							    ...
							    if( tmp != null ) {									
									...
									$("#company", opener.document).append("<option value='" + tmp + "'>" +tmp + "</option>");
