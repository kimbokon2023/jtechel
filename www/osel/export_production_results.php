...
try {
    ...
    $data = [
        ...
        'H' => $selected_data['car_structure'] ?? '일반형',
        'I' => $selected_data['material_type'] ?? '',
        'J' => $ipark_check,
        'K' => $selected_data['material_thickness'] ?? '',
        // 제작 조건 설정
        'L' => $project_type,
        ...
        // 패널 정보
        'S' => $production_results['total_panels'] ?? 0,
        'T' => $selected_data['notes'] ?? ''
    ];
    ...
    if (!empty($display_panel_data)) {
        foreach ($display_panel_data as $panel_num => $panel_info) {
            ...
            $productionSheet->setCellValue('I' . $productionRow, $selected_data['car_structure'] ?? '일반형');
            $productionSheet->setCellValue('J' . $productionRow, $ipark_check);
            $productionSheet->setCellValue('K' . $productionRow, $selected_data['material_type']);
            $productionSheet->setCellValue('L' . $productionRow, $selected_data['material_thickness'] == 0 ? '' : $selected_data['material_thickness']);
            ...
            if ($production_settings['molding_included'] && is_numeric($panel_num) && $panel_num >= 2 && $panel_num <= 10) {
                ...
                $is_pass_through = ($selected_data['car_structure'] ?? '일반형') === '관통형';
            ...
            elseif (isset($panel_info['holes']) && is_array($panel_info['holes'])) {
                ...
                foreach ($panel_info['holes'] as $hole) {
                    if (isset($hole['width']) && isset($hole['height'])) {
                        $hole_width_val = $hole['width'];
                        $hole_height_val = $hole['height'];
                        
                        // 아이파크 프로젝트인 경우 제작치수 적용
                        if ($is_ipark_project) {
                            $hole_width_val = $hole_width_val - 70;  // 제작가로 = 실측가로 - 70
                            $hole_height_val = $hole_height_val - 13; // 제작세로 = 실측세로 - 13
                        }
                        
                        $hole_details[] = $hole_width_val . '×' . $hole_height_val;
                    }
                    if (isset($hole['floor_height'])) {
                        $floor_height_val = $hole['floor_height'];
                        
                        // 아이파크 프로젝트인 경우 제작치수 적용
                        if ($is_ipark_project) {
                            $floor_height_val = $floor_height_val + 6; // 제작바닥높이 = 실측바닥높이 + 6
                        }
                        
                        $floor_heights[] = $floor_height_val;
                    }
                    if (isset($hole['entrance_distance'])) {
                        $entrance_distance_val = $hole['entrance_distance'];
                        
                        // 아이파크 프로젝트인 경우 제작치수 적용
                        if ($is_ipark_project && isset($panel_info['width'])) {
                            $production_width = $panel_info['width']; // 제작패널전체폭
                            $production_drilling_width = $hole_width_val ?? $hole['width'] - 70; // 제작가로크기확정
                            $entrance_distance_val = ($production_width - $production_drilling_width) / 2; // 제작출입구위치
                        }
                        
                        $entrance_distances[] = $entrance_distance_val;
                    }
                }
            ...
            $productionSheet->setCellValue('R' . $productionRow, $hole_width);  // 타공 가로
            $productionSheet->setCellValue('S' . $productionRow, $hole_height); // 타공 세로
            $productionSheet->setCellValue('T' . $productionRow, $hole_floor_height); // 타공 높이(밑기준)
            $productionSheet->setCellValue('U' . $productionRow, $hole_entrance_distance); // 입구방향에서 떨어짐
        ...
        if (!empty($transomData)) {
            ...
            $productionSheet->setCellValue('Y' . $productionRow, '');
